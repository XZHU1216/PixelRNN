...
class Config:
    ...
    batch_size_test = 1024  # 128
    criterion = nn.BCELoss()  # Binary Cross-Entropy Loss
    epochs = 10
    lr = 1e-3
    L = 1e-5  # coefficient Regularization
    dropout = 0.3
...
class RowLSTMLayer(nn.Module):
    ...
    def forward(self, inputs, states):
        ...
        inputs = inputs.view(B, C, W)  # Now (Batch, Channels, Width)
...
def test_nll(model, test_loader, dataset_type="mnist"):
    ...
    if dataset_type == "cifar10":
        bits_per_dim = mean_nll / (32 * 32 * 3 * np.log(2))
...
if __name__ == "__main__":
    ...
    num_layers_list = [4, 7, 9]

    # Create all possible combinations
    hyperparameter_combinations = list(itertools.product(
        learning_rates, regularizations, dropouts, num_layers_list
    ))

    # Store results
    results = []

    for lr, L, dropout, num_layers in hyperparameter_combinations:
        print(f"\nTesting config: lr={lr}, L={L}, dropout={dropout}, layers={num_layers}")
        Config.learning_rate = lr
        Config.dropout = dropout
        Config.L = L

        # Initialize model
        model = PixelRNN_model(num_layers=num_layers, num_filters=16, input_size=32, type_rnn='diagonal').to(
            device)
        optimizer = optim.RMSprop(model.parameters(), lr=lr, weight_decay=L)

        # Train for a few epochs
        train_losses, val_losses = train(
            model, train_loader, val_loader, epochs=2, optimizer=optimizer,
            device=device, criterion=Config.criterion, checkpoint_path=None
        )
        ...
        # Save results
        results.append([lr, L, dropout, num_layers, train_losses[-1], val_losses[-1], test_loss])

        print(f"Final Test Loss: {test_loss:.4f}")

    # Convert results to DataFrame
    results_df = pd.DataFrame(results, columns=[
        "Learning Rate", "Regularization", "Dropout", "Num Layers", "Train Loss", "Validation Loss",
        "Test Loss"
    ])

    results_df.to_csv("hyperparameter_results_full.csv", index=False)
    print("Résultats sauvegardés dans hyperparameter_results_full.csv")
